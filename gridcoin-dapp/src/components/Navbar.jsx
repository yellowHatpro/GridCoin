...
const Navbar = () => {
  ...
  return (
    <div className="ml-30 text-purple-200 flex md:flex-row lg:flex-row sm:flex-row flex-col-reverse justify-between mb-2 gap-6 min-w-screen">
      <Link to={"/"} className={"sm:text-4xl text-3xl font-thin"}>
        GridCoinStore
      </Link>
      <div className="sm:flex hidden flex-row justify-end items-center w-full gap-4">
        <div className={"flex font-light text-xl flex-row gap-3"}>
          <Link to={"/settings"}>
            <button
              className={
                "rounded-full bg-[#1e1e2e] transition-all duration-500 p-2 text-purple-200 hover:text-[#1e1e2e] hover:bg-purple-200"
              }
            >
              <SettingsIcon />
            </button>
        ...
        {!address && (
          <button
            className={
              "bg-[#1e1e2e] p-2 rounded-md hover:bg-purple-200 hover:text-[#1e1e2e] transition-all duration-500"
            }
            onClick={() => {
        ...
        {address && (
          <Link to="/profile">
            <div
              className={
                "w-[40px] h-[40px] rounded-full bg-[#2c2f32] flex justify-center items-center cursor-pointer"
              }
            >
              <img
                src={fallbackUser}
      ...
      <div className="sm:hidden flex justify-between items-center relative">
        ...
        <div
          className={`text-purple-200 absolute top-[60px] right-0 left-0 bg-[#1c1c24] border border-purple-200 rounded-md z-10 shadow-secondary p-4 ${
