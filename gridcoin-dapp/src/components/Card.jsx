...
import {Check, CheckCircle, MinusCircle, PlusCircle, ShoppingCart, TrashIcon} from "lucide-react";
...
const Card = ({item}) => {
    ...
    const itemAlreadyPresentQuantity = (isInCart) ? userContext.cart.find(cartItem=>cartItem.id===id).quantity : 0
        ...
        <div
            ...
            <div className={"p-2"}>
                ...
                <div className={"text-purple-200 flex flex-row max-w-fit gap-2"}>
                    ...
                    <h1 disabled={true}>{selectedAmount}</h1>
                    <PlusCircle
                        className={"hover:bg-purple-200 hover:text-[#13131a] rounded-full"}
                        onClick={() => {
